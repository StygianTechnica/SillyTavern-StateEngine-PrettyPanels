...
- **Assets** - images uploaded with the theme (backdrop, panelTexture, accentStripe,
  cornerGlyph, clockFace, hourHand, minuteHand, secondHand, or any name you like). Each is
  stored as a Pretty Panels variable `pp_theme_<themeId>_<assetName>` in
  `extensionSettings.prettyPanelsVariables`, and `theme.assets` maps the asset name to it.
  Uploads are scaled to fit 1024px and re-encoded as WebP (transparency kept; SVGs stay
  vector), up to 1.5 MB. Replacing an asset updates everything using it.
...
always adds a new theme. Asset images travel inside the exported file; duplicating or
importing a theme gives its assets their own variables under the new theme's id, and
deleting a theme deletes its assets. Panel templates keep their theme and variant.
...
| `src/ui/image-upload.js` | Image upload: pick, scale down, re-encode |
